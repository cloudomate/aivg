...
## 6. Transports: WebRTC and gRPC (feature 022)

The voice plane runs over one of two transports behind a single internal seam
(`src/transport/transport.hpp`); the public API is identical either way.

| Transport | Tier | Build flag | Notes |
|---|---|---|---|
| **WebRTC** (libpeer) | RPi + ESP32-S3 | `AIVG_SAT_ENABLE_VOICE=ON` | default; Opus @ 48 kHz; needs the libpeer DTLS-client patch |
| **gRPC** (`Audio.Stream`) | RPi/POSIX only | `AIVG_SAT_ENABLE_GRPC=ON` | raw 16 kHz PCM up (no on-device encode); codec-tagged audio + transcripts + events down; no ICE/DTLS to stall |

gRPC consumes the canonical contract at repo-root `proto/aivg/satellite/v1/`
(shared with the gateway, feature 021). C++ stubs are generated by
`cmake/GenerateProto.cmake` and **checked into `src/grpc/_generated/`** — a
consumer build needs no `protoc`. The gRPC stack (`grpc++`/`protobuf`) is
**POSIX-tier only**: the ESP-IDF component never links it (see
`specs/022-cpp-sdk-grpc-transport/esp32-grpc-spike.md`).

### Selecting a transport

```cpp
SatelliteOptions o;
o.transport = TransportPref::Auto;   // negotiate with the gateway (default)
// o.transport = TransportPref::Grpc;   // pin gRPC (errors if not compiled in)
// o.transport = TransportPref::Webrtc; // pin WebRTC
o.grpc_port  = 8645;                  // gateway gRPC audio port
o.grpc_tls   = false;                 // insecure (trusted LAN) | true => SSL/mTLS
```

`Auto` advertises the build's compiled-in transports (`transport_capabilities`)
in the register frame and uses the gateway's `chosen_transport`. A WebRTC-only
build advertises only `["webrtc"]`, so a gateway never steers it to gRPC.

### Source compatibility (SC-005)

A `-DAIVG_SAT_ENABLE_GRPC=OFF` build is behaviour-identical to feature 020:
existing integrations compile and run with **no source change** — the gRPC
transport is additive and opt-in.
